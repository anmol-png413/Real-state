...
Locator:  locator('.hidden.lg\\:flex form').first()
Expected: visible
Received: hidden
Timeout:  8000ms
...
Call log:
  ...
  - waiting for locator('.hidden.lg\\:flex form').first()
    19 × locator resolved to <form>…</form>
...
- text: "RERA: UPRERAPRJ466336"
- heading "AU Cosmos Corner" [level=1]
- paragraph: Siddharth Vihar, Ghaziabad · G+40 · 5.6 Acres
- button "3 BHK from ₹1.22 Cr*"
- button "5 BHK from ₹2.19 Cr*"
- textbox "Your Name"
- textbox "Phone Number"
- button "I'm Interested In... ▼"
- button "Book Free Site Visit →"
- paragraph: 🔒 100% secure · No spam
- img "AU Cosmos Corner exterior view Siddharth Vihar Ghaziabad"
- img "AU Cosmos Corner 3 BHK flat interior Ghaziabad"
- img "AU Cosmos Corner amenities clubhouse Ghaziabad"
- button "‹"
- button "›"
- button
- button
...
- contentinfo:
  ...
  - paragraph: © 1970 Truelite Estates LLP. Authorized Channel Partner of AU Real Estate Pvt. Ltd.
...
```ts
  196 | 
  197 |     await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
  198 |     const footerBtn = page.locator("footer").getByRole("button", { name: /book|get/i }).first();
  199 |     await expect(footerBtn).toBeVisible({ timeout: 10_000 });
  200 |     await footerBtn.click();
  201 | 
  202 |     const modal = page.locator(".fixed.inset-0").first();
  203 |     await expect(modal).toBeVisible({ timeout: 5_000 });
  204 |     await fillAndSubmit(modal, { name: testName("TC-09 Footer") });
  205 | 
  206 |     const { status, body } = await apiDone;
  207 |     expect(status).toBe(200);
  208 |     expect(body.success).toBe(true);
  209 |   });
  210 | 
  211 |   // ── TC-10 ──────────────────────────────────────────────────────────────
