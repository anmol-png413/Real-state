...
Locator: locator('.fixed.inset-0').first().locator('p').filter({ hasText: /name|required/i }).first()
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('.fixed.inset-0').first().locator('p').filter({ hasText: /name|required/i }).first()
